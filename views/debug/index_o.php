...
<table class="layui-table mt10 mb150">
    ...
    {
        foreach ($dir as $n => $rs) {
//            if ($lev === 0 and strtotime($n) < (time() - 86400 * 2)) continue;

            $span = str_repeat("<span class='lev'>---------------</span>", $lev);
            if (empty($rs)) {
                $url = urlencode("{$host}/{$n}");
                $bar = "<a href='/debug/files/{$url}' width='1600' height='750' title='{$host}/{$n}' class='open layui-btn layui-btn-xs'>{$host}/{$n}</a>";
